...
def read_sensors(device):
	device.read(READ_ANALOG_CHANNELS, len(READ_ANALOG_CHANNELS), READ_ENCODER_CHANNELS, len(READ_ENCODER_CHANNELS), None, 0, 
				READ_OTHER_CHANNELS, len(READ_OTHER_CHANNELS), readAnalogBuffer, readEncoderBuffer, None, readOtherBuffer)
...
try:
	...
	input("Press enter to start reading sensors\nctrl+C to quit")
	...
	try:
		...
		while True:
			read_sensors(aero)
			motorCurrent = readAnalogBuffer
			motorPosition = 2*np.pi*readEncoderBuffer[0:2]/2048
			pitchAngle = 2*np.pi*readEncoderBuffer[2]/2048
			yawAngle = 2*np.pi*readEncoderBuffer[3]/4096
			gyroscope = readOtherBuffer[0:3]
			accelerometer = readOtherBuffer[3:6]
			motorSpeed = 2*np.pi*readOtherBuffer[6:8]/2048
			pitchRate = 2*np.pi*readOtherBuffer[8]/2048
			yawRate = 2*np.pi*readOtherBuffer[9]/4096

			print(f"Pitch: {pitchAngle: 7.2f} rad, Rate: {pitchRate: 7.2f} rad/s")
			print(f"Yaw:   {yawAngle: 7.2f} rad, Rate: {yawRate: 7.2f} rad/s")
			print(f"Motor0: {motorSpeed[0]: 7.2f} rad/s, Position: {motorPosition[0]: 7.2f} rad, Current: {motorCurrent[0]: 7.2f} A")
			print(f"Motor1: {motorSpeed[1]: 7.2f} rad/s, Position: {motorPosition[1]: 7.2f} rad, Current: {motorCurrent[1]: 7.2f} A")
			print(f"Acceleration x: {accelerometer[0]: 7.2f} m/s^2, y: {accelerometer[1]: 7.2f} m/s^2, z: {accelerometer[2]: 7.2f} m/s^2")
			print(f"Gyroscope x:    {gyroscope[0]: 7.2f} rad/s, y: {gyroscope[1]: 7.2f} rad/s, z: {gyroscope[2]: 7.2f} rad/s")
			print("\n")
